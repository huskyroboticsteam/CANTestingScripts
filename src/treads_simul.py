...
try:
    ...
    write_to_serial(f"1 07 13 00 00")
    write_to_serial(f"1 07 03 00 00")
    ...
    scaled = 0
    while True:
        motor = input("Motor SN: ")
        SN = "03" if motor == "vert" else "13"
        print(SN)
        ...
        write_to_serial(f"1 07 {SN} 03 {data_post}")

    # start_time = time.time()

    # while (time.time() - start_time < SECONDS):
        # write_to_serial(f"1 04 04 03 {data_post}")
        # write_to_serial(f"1 04 {R_SN} 03 {data_post}")
